...
extern int g_verbosity;
extern int g_sanity_check_level;
extern bool g_persist;
extern bool g_do_fsync;
extern size_t g_log_size_limit;
extern size_t g_keep_history_count;

void kvt_set_persist_param(size_t log_size_limit, size_t keep_history_count);
void kvt_set_persist(bool persist, bool do_fsync = false);
...
class KVTLogger 
{
    private:
        ...
        static std::string from_hex_string(const std::string& hex_str) {
            std::string result;
    ...
    public:
        KVTLogger(const std::string& file_name) 
            : next_log_id_(1) {
            total_size_ = 0;
            ...
            if (ofs_.seekp(0, std::ios::end)) { //continue append to a log file. 
                total_size_ = ofs_.tellp();
            }
        }
        ...
        void flush_log() {
            ...
            // format: ID LENGTH CHECKSUM PAYLOAD
        ...
        static bool read_entry_from_file(std::ifstream & ifs, 
            ...
            payload = from_hex_string(hex_payload);
            
            if (payload_length != payload.length()){
                std::cout << "Corrupt log entry: payload length mismatch" << std::endl; 
                return false; // payload length mismatch
            }
            // Verify checksum
            uint32_t calculated_checksum = calculate_checksum(payload.c_str(), payload.length());
            if (calculated_checksum != checksum) {
                std::cout << "Corrupt log entry: checksum mismatch" << std::endl; 
...
class KVTWrapper
{
    private:
        ...
        std::string check_point_name_base_ = "/kvt_data.ckpt";
        std::string log_file_name_base_ = "/kvt_data.log";
    ...
    public:
        ...
        void startup() {
            ...
            for (const auto& entry : std::filesystem::directory_iterator(data_path_)) {
                ...
                if (filename.find(check_point_name_base_) != std::string::npos) {
            ...
            for (const auto& entry : std::filesystem::directory_iterator(data_path_)) {
                std::string filename = entry.path().filename().string();
                if (filename.find(log_file_name_base_) != std::string::npos) {
                    size_t id = get_checkpoint_id_from_file_name(entry.path().string());
            ...
            try {
                logger_ = std::make_unique<KVTLogger>(log_file_name_str);
        ...
        bool process_log_entry(const std::string& payload)
        {
            ...
            if (operation == "CREATE_TABLE") {
                ...
                } else if (result == KVTError::TABLE_ALREADY_EXISTS) {
                    // Table already exists - this is fine during replay
                    std::cout << "Replay CREATE_TABLE: Table already exists (skipping)" << std::endl;
                } else {
                ...
                std::string error_msg;
                KVTError result = get(tx_id, table_id, key, value, error_msg);
                if (result != KVTError::SUCCESS) {
                    std::cout << "Replay GET failed: " << error_msg << std::endl;
                    return false;
                }
                return true;
            }
                ...
                std::cout << "Replay SKIP: " << operation << std::endl;
                return true;
            }
            else {
        ...
        bool replay_log(std::string log_file_name)
        {
            ...
            while (ifs.good()) {
                std::string payload;
                uint64_t log_id;
                if (!KVTLogger::read_entry_from_file(ifs, log_id, payload)) {
                    ...
                    std::cout << "Failed to read log entry from file: " << log_file_name << std::endl;
                    return false;
                }
                // Process the payload using common function
                if (!process_log_entry(payload)) {
                    std::cout << "Failed to process log entry: " << log_file_name << std::endl;
                    return false;
                }
            }
            ifs.close();
        ...
        {
            if (logger_->get_total_payload_size() <= g_log_size_limit) {
                return; 
            }
            ...
            try {
                logger_ = std::make_unique<KVTLogger>(get_logfile_name(check_point_id_ - 1));
            } catch (const std::exception& e) {
                std::cout << "Failed to open log file: " << get_logfile_name(check_point_id_ - 1) << " - " << e.what() << std::endl;
                exit(1);
            }
            for (size_t i = 0; i< 10; ++i){
                int64_t id = check_point_id_ - i - g_keep_history_count;
                if (id < 0)
                    continue;
